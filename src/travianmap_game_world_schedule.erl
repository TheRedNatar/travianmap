...
parse_body(Body) ->
    ...
    FilteredLines = lists:filter(fun(X) -> binary:match(X, <<"https://tracking.traviangames.com">>) /= nomatch end, AllLines),
    ServerList = lists:map(fun(X) -> get_server_url_from_redirect(get_redirect_url(X)) end, FilteredLines),
    lists:map(fun({_, ServerUrl}) -> ServerUrl end, lists:filter(fun({Atom, _}) -> Atom == ok end, ServerList)).
...
get_server_url_from_redirect(TableUrl) when is_binary(TableUrl)->
    ...
    IsLocation = fun ({"location", _}) -> true;
		      (_) -> false end,
